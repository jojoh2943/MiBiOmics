...
write_report <- function(filePath, datasets, vector_input){
         ...
         "S"={
           ...
           for (col in 1:ncol(datasets[["sampleAnnot"]])){
             PCoA_text <- paste(PCoA_text, "\n### ", colnames(datasets[["sampleAnnot"]])[col],
                                ...
                                "geom_point(size = 2) +\n",
                                "labs(title=paste('PCoA realized with the distance:', list_input[['dist_PCoA_P2']]), x = sprintf('Axis 1 [%s%% Variance]', 100 * round(dudi_PCoA$eig[1] / sum(dudi_PCoA$eig), 2)),\n",
         ...
         "M"={
           ...
           for (col in 1:ncol(datasets[["sampleAnnot"]])){
             PCoA_text_D2 <- paste(PCoA_text_D2, "\n### ", colnames(datasets[["sampleAnnot"]])[col],
                                ...
                                "geom_point(size = 2) +\n",
                                "labs(title=paste('PCoA realized with the distance:', list_input[['dist_PCoA_D2_P2']]), x = sprintf('Axis 1 [%s%% Variance]', 100 * round(dudi_PCoA_D2$eig[1] / sum(dudi_PCoA_D2$eig), 2)),\n",
  ...
  if (vector_input[6] == "Yes" | datasets[["type"]] == "ST"){
    writeLines(c("---",
                 ...
                 "sampleAnnot <- sampleAnnot[which(rownames(sampleAnnot) %in% rownames(exprDat)),]",
                 "exprDat <- exprDat[match(rownames(sampleAnnot), rownames(exprDat)),]",
                 "sampleID <- rownames(sampleAnnot)",
                 ...
                 "### SELECTED FILTRATION",
                 "if (list_treatment[['filtration']] == 'FP'){",
                 "  if(list_treatment[['filtration_value']] != 0){",
                 "    Prevalence_threshold = round((list_treatment[['filtration_value']]/100) * nrow(exprDat))",
                 "    names_OTU <- c()",
                 ...
                 "   }",
                 "}",
                 "if (list_treatment[['filtration']] == 'FC'){",
                 "    min_count = list_treatment[['filtration_value']]",
                 "    exprDat <- exprDat[, which(colSums(exprDat) >= min_count)]",
                 "  }",
                 
                 "# DATA NORMALIZATION",
                 "if (list_treatment[['normalization']] == 'MinMax'){",
                 "    exprDat <- as.data.frame(lapply(exprDat, normalize))",
                 "  }else{",
                 "    if (list_treatment[['normalization']] == 'Z'){",
                 "      exprDat <- scale(exprDat)",
                 "    }",
                 "  }",
                 
                 "# DATA TRANFORMATION",
                 "if (list_treatment[['transformation']] != 'NONE' ){",
                 "  if (list_treatment[['transformation']] == 'Log10'){",
                 "    exprDat <- log((1 + exprDat), base = 10)",
                 "  }else{",
                 "    if(list_treatment[['transformation']] == 'Log2'){",
                 "      exprDat <- log((1 + exprDat), base = 2)",
                 "    }else{",
                 "      if(list_treatment[['transformation']] == 'Square'){",
                 "        exprDat <- exprDat^2",
                 "      }else{",
                 "        if(list_treatment[['transformation']] == 'sqrt'){",
                 "          exprDat <- sqrt(exprDat)",
                 "        }else{",
                 "          if(list_treatment[['transformation']] == 'Hellinger'){",
                 ...
                 "  rel_ab_analysis <- 'Yes'",
                 "}",
                 "if (list_treatment[['SampleToRemove']] != 'NONE'){",
                 "  exprDat <- exprDat[which(rownames(exprDat) %!in% list_treatment[['SampleToRemove']]),]",
                 "  sampleAnnot <- sampleAnnot[which(rownames(sampleAnnot) %!in% list_treatment[['SampleToRemove']]),] ",
                 "  exprDat <- exprDat[match(rownames(sampleAnnot), rownames(exprDat)),]",
                 ...
                 "```{r, echo = FALSE, message = FALSE, warning = FALSE, include = FALSE}",
                 "D1_dist <- vegdist(exprDat, method = list_input[['dist_PCoA_P2']])",
   ...
   paste("\n\n", vector_input[16], "\n \n")
    ...
    if (vector_input[7] == "Yes" | datasets[["type"]] == "MT"){
      writeLines(c("---",
                   ...
                   "sampleAnnot <- sampleAnnot[which(rownames(sampleAnnot) %in% rownames(exprDatSec)),]",
                   "exprDat <- exprDat[which(rownames(exprDat) %in% rownames(sampleAnnot)),]",
                   "exprDatSec <- exprDatSec[match(rownames(sampleAnnot), rownames(exprDatSec)),]",
                   "sampleID <- rownames(sampleAnnot)",
                   ...
                   "}",
                   "### SELECTED FILTRATION",
                   "if (list_treatment[['filtrationD2']] == 'FP'){",
                   "  if(list_treatment[['filtration_valueD2']] != 0){",
                   "    Prevalence_threshold = round((list_treatment[['filtration_valueD2']]/100) * nrow(exprDatSec))",
                   "    names_OTU <- c()",
                   ...
                   "   }",
                   "}",
                   "if (list_treatment[['filtrationD2']] == 'FC'){",
                   "    min_count = list_treatment[['filtration_valueD2']]",
                   "    exprDatSec <- exprDatSec[, which(colSums(exprDatSec) >= min_count)]",
                   "  }",
                   
                   "# DATA NORMALIZATION",
                   "if (list_treatment[['normalizationD2']] == 'MinMax'){",
                   "    exprDatSec <- as.data.frame(lapply(exprDatSec, normalize))",
                   "  }else{",
                   "    if (list_treatment[['normalizationD2']] == 'Z'){",
                   "      exprDatSec <- scale(exprDatSec)",
                   "    }",
                   "  }",
                   
                   "# DATA TRANFORMATION",
                   "if (list_treatment[['transformationD2']] != 'NONE' ){",
                   "  if (list_treatment[['transformationD2']] == 'Log10'){",
                   "    exprDatSec <- log((1 + exprDatSec), base = 10)",
                   "  }else{",
                   "    if(list_treatment[['transformationD2']] == 'Log2'){",
                   "      exprDatSec <- log((1 + exprDatSec), base = 2)",
                   "    }else{",
                   "      if(list_treatment[['transformationD2']] == 'Square'){",
                   "        exprDatSec <- exprDatSec^2",
                   "      }else{",
                   "        if(list_treatment[['transformationD2']] == 'sqrt'){",
                   "          exprDatSec <- sqrt(exprDatSec)",
                   "        }else{",
                   "          if(list_treatment[['transformationD2']] == 'Hellinger'){",
                   ...
                   "}",
                   "if (list_treatment[['SampleToRemove']] != 'NONE'){",
                   "  exprDatSec <- exprDatSec[which(rownames(exprDatSec) %!in% list_treatment[['SampleToRemove']]),]",
                   "  exprDatSec <- exprDatSec[match(rownames(sampleAnnot), rownames(exprDatSec)),]",
                   "}",
                   "```",
                   ...
                   "```{r, echo = FALSE, message = FALSE, fig.height = 10, fig.width = 10}",
                   "D2_dist <- vegdist(exprDatSec, method = list_input[['dist_PCoA_D2_P2']])",
           ...
           # "```{r}",
           # "print(geneTraitSignificance_D2)",
           # "```",
           # 
           # "```{r}",
           # "print(geneModuleMembership_D2)",
           # "```"
           TMM_D2_P4,
           paste("\n\n", vector_input[18], "\n \n"),
           #----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      "The following plot represent the correlation between each external trait and each module of the WGCNA network. The heatmap can help finding",
      " modules specific to a specific traits, find the groups of variables associated with a shift in the concentration of one parameter.\n\n",
      "```{r, echo = FALSE, fig.height = 15, fig.width = 15, warning = FALSE, message = FALSE}",
      "nSamples <- nrow(exprDat)",
      "text_matrix_vector <- c()",
      "for (condition in 1:(ncol(sampleAnnot))){",
      "  condition_name <- colnames(sampleAnnot)[condition]",
      ...
      paste("\n\n", vector_input[16], "\n \n")
        ...
        writeLines(c("---",
                     ...
                     "      printFlush(paste('Removing samples:', paste(rownames(exprDatSec)[!gsg$goodSamples], collapse = ', ')));",
                     "    }",
                     "    exprDatSec <- exprDatSec[rownames(v)[gsg$goodSamples], colnames(exprDatSec)[gsg$goodGenes]]",
                     "  }",
                     "}",
                     "### SELECTED FILTRATION",
                     "if (list_treatment[['filtrationD2']] == 'FP'){",
