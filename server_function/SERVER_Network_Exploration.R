...
MEs_orderedTer <- reactive ({
  MEs <- data.frame(selectedMEs3(), rownames(sampleAnnot_ter()))
  colnames(MEs) <- c(colnames(selectedMEs3()), "sampleName")
  if (input$selectVariable_barplotTer != "unordered"){
    MEs <- MEs[order(sampleAnnot_ter()[,input$selectVariable_barplotTer]),]
  }
  MEs$sampleName <- as.character(MEs$sampleName)
  MEs$sampleName <- factor(MEs$sampleName, levels = unique(MEs$sampleName))
  MEs
...
sampleAnnot_orderedTer <- reactive({
  sampleAnnot <- data.frame(sampleAnnot_ter(), rownames(sampleAnnot_ter()))
  colnames(sampleAnnot) <- c(colnames(sampleAnnot_ter()), "sampleName")
  if (input$selectVariable_barplotTer != "unordered"){
    sampleAnnot <- sampleAnnot[order(sampleAnnot_ter()[,input$selectVariable_barplotTer]),]
  }
  sampleAnnot$sampleName <- as.character(sampleAnnot$sampleName)
  sampleAnnot$sampleName <- factor(sampleAnnot$sampleName, levels = unique(sampleAnnot$sampleName))
  sampleAnnot
  ...
  for (i in names(sampleAnnot_2())) {
    if(length(unique(sampleAnnot_2()[,i])) > 1){
      ...
      if (i == colnames(sampleAnnot_2())[1]){
        geneTraitSignificance = as.data.frame(cor(exprDat_WGCNA(), env, method= as.character(input$selectCorrelation), use="na.or.complete"));
      }else{
        geneTraitSignificance = data.frame(geneTraitSignificance, cor(exprDat_WGCNA(), env, method= as.character(input$selectCorrelation), use="na.or.complete"));
      }
    }
  }
...
geneTraitSignificanceTer <- reactive({
  colnames_used <- c()
  for (i in names(sampleAnnot_ter())) {
    if(length(unique(sampleAnnot_ter()[,i])) > 1){
      if (is.numeric(sampleAnnot_ter()[,i])){
        env = as.data.frame(sampleAnnot_ter()[,i])
      }else{
        env = as.data.frame(as.numeric(as.factor(sampleAnnot_ter()[,i])))
      }
      colnames(env) = i
      colnames_used <- c(colnames_used, i)
      modNames = substring(names(selectedMEs3()), 3)
      if (i == colnames(sampleAnnot_ter())[1]){
        geneTraitSignificance = as.data.frame(cor(exprDatTer_WGCNA(), env, method= as.character(input$selectCorrelationTer), use="na.or.complete"));
      }else{
        geneTraitSignificance = data.frame(geneTraitSignificance, cor(exprDatTer_WGCNA(), env, method= as.character(input$selectCorrelationTer), use="na.or.complete"));
      }
    }
  }
  colnames(geneTraitSignificance) <- paste("GS.", colnames_used, sep = "")
...
moduleMembershipTer <- reactive ({
  module = input$selectModuleTer
  column = paste("MM",module, sep = "")
  column2 = paste("GS.", input$selectVariable1Ter, sep = "")
  moduleGenes = (selectedDynamicColor3()==module)
  a <- abs(geneModuleMembershipTer()[moduleGenes, column])
  b <- abs(geneTraitSignificanceTer()[moduleGenes, column2])
  c <- data.frame(a,b)
  colnames(c) <- c("moduleMembership", "TraitSignificance")
  c
})
...
textModuleMembershipTer <- reactive({
  module = input$selectModuleTer
  column = paste("MM",module, sep = "")
  column2 = paste("GS.", input$selectVariable1Ter, sep = "")
  moduleGenes = (selectedDynamicColor3()== input$selectModuleTer)
  pv = corPvalueStudent(cor(abs(geneModuleMembershipTer()[moduleGenes, column]), abs(geneTraitSignificanceTer()[moduleGenes, column2]), method= input$selectCorrelationTer, use="na.or.complete"), length(geneTraitSignificanceTer()[moduleGenes, column2]))
  Correlation <- cor(abs(geneModuleMembershipTer()[moduleGenes, column]), abs(geneTraitSignificanceTer()[moduleGenes, column2]))
  text_scatter = paste("Corr = ", round(Correlation, 2), "\nP-val = ", pv)
  text_scatter
})
...
# Recalculate MEs with color labels
selectedCondDFTer <- reactive({
  moduleColors = selectedDynamicColor3()
  nSamples <- nrow(exprDatTer_WGCNA())
  MEs0 = moduleEigengenes(exprDatTer_WGCNA(), moduleColors)$eigengenes
  MEs = orderMEs(MEs0)
  text_matrix_vector <- c()
  pval_vector <- c()
  corr_vector <- c()
  for (condition in 1:(ncol(sampleAnnot_ter()))){
    ...
    if (is.numeric(sampleAnnot_ter()[,condition_name])){
      ...
      sampleAnnot4 <- sampleAnnot_ter()
      sampleAnnot4[,condition_name] <- as.factor(sampleAnnot4[, condition_name])
      sampleAnnot4[,condition_name] <- as.numeric(sampleAnnot4[,condition_name])
      moduleTraitCor = cor(selectedMEs3(), sampleAnnot4[,condition_name], use = "p", method = input$selectCorrelationTer)
    }
    moduleTraitPvalue = corPvalueStudent(moduleTraitCor, nSamples)
    pval_vector <- c(pval_vector, moduleTraitPvalue)
    corr_vector <- c(corr_vector, moduleTraitCor)
...
markers.data_D3 <- reactive({
  markers <- colnames(exprDatTer_WGCNA())[selectedDynamicColor3()==input$colorModule_D3]
  exprDatTer_WGCNA()[,markers]
})

env.markers.data_D3 <- reactive({
  sampleAnnot <- sampleAnnot_ter()
  if (!is.numeric(sampleAnnot[,input$sampleAnnotSelection_D3])){
    sampleAnnot[,input$sampleAnnotSelection_D3] <- as.numeric(as.factor(sampleAnnot[,input$sampleAnnotSelection_D3]))
  }
  env = as.data.frame(sampleAnnot[,input$sampleAnnotSelection_D3])
  colnames(env) = input$sampleAnnotSelection_D3
  data.frame(cbind(markers.data_D3(), env))
})
...
fit.object.ncomp_D3 <- reactive({
  ...
  if (length(colnames(exprDatTer_WGCNA())[selectedDynamicColor3()==input$colorModule_D3]) >= 10){
    ...
    ncomp=length(colnames(exprDatTer_WGCNA())[selectedDynamicColor3()==input$colorModule_D3])
  }
  formulaChar <- paste(input$sampleAnnotSelection_D3, "~.", sep ="")
  fit = plsr(formula(formulaChar), data=env.markers.data_D3(), validation="LOO", method = "oscorespls", ncomp=ncomp)
  fit
})

fit.object_D3 <- reactive({
  ncomp=input$ncomponent_D3
  formulaChar <- paste(input$sampleAnnotSelection_D3, "~.", sep ="")
  fit = plsr(formula(formulaChar), data=env.markers.data_D3(), validation="LOO", method = "oscorespls", ncomp=ncomp)
  fit
})
...
cvs.object_D3 <- reactive({
  ncomp = input$ncomponent_D3
  fit <- fit.object_D3()
  env.markers.data <- env.markers.data_D3()
  cvs = round( cor(fit$val$pred[,,ncomp], env.markers.data[[input$sampleAnnotSelection_D3]][!is.na(env.markers.data[[input$sampleAnnotSelection_D3]])]), digits=3);
  cvs
})
...
vip.df_D3 <- reactive({
  ...
  vip = round(vip, digits=2)
  o = order(vip, decreasing=TRUE)
  df <- data.frame(Marker = colnames(markers.data_D3()), VIP = vip, Temp.Cor = geneTraitSignificanceTer()[colnames(markers.data_D3()),])
  df
})

#### HIVE PLOT

edge_to_node_D3 <- reactive({
  module <- input$colorModule_D3
  probes <- colnames(exprDatTer_WGCNA())
  inModule <- is.finite(match(selectedDynamicColor3(), module))
  modProbes <- probes[inModule]
  modTOM <- selectedDissTOM3()[inModule, inModule]
  dimnames(modTOM) <- list(modProbes, modProbes)
  cyt <- exportNetworkToCytoscape(modTOM,
                                  edgeFile = NULL,
                                  nodeFile = NULL,
                                  weighted = TRUE,
                                  nodeNames = modProbes,
                                  nodeAttr = selectedDynamicColor3()[inModule])
  cyt
})

df.hive.plot_D3 <- reactive({
  vip.df <- vip.df_D3()
  col <- paste("Temp.Cor.GS.", input$sampleAnnotSelection_D3, sep = "")

    if (input$OmicTable3 == "OTUs" && input$TaxonFile1){
      Annot <- taxTable1()[rownames(vip.df), input$taxAnnotTer]
    }else{
      Annot <- rownames(vip.df)
    }
...
df.hive.to.plot_D3 <- reactive({
  edge_node <- edge_to_node_D3()
  edgeData <- edge_node$`edgeData`
  vector_geom_curve(edgeData, df.hive.plot_D3())
})
...
output$sampleAnnotSelection_D3 <- renderUI({
  selectInput("sampleAnnotSelection_D3",
              label = "Choose a variable: ",
              choices = colnames(sampleAnnot_2()),
              selected = colnames(sampleAnnot_2())[2])
})

output$colorModule_D3 <-  renderUI({
  selectInput("colorModule_D3",
              label= "Choose a module color: ",
              choices = substr(names(selectedMEs3()), 3, 40))
})
...
output$SelectModuleTer <- renderUI({
  selectInput("selectModuleTer",
              label= "Choose a module color: ",
              choices = substr(names(selectedMEs3()), 3, 40))
})
...
output$taxAnnotTer <- renderUI({
  selectInput("taxAnnotTer",
              label = "Select a taxonomic level for the relative abundance plot",
              choices = colnames(taxTable_present()),
              selected = colnames(taxTable_present())[3])
})

output$SelectTaxo1Ter <- renderUI({
  selectInput("selectTaxo2Ter",
              label = "Select a taxonomic level for the relative abundance plot",
              choices = colnames(taxTable_present()),
              selected = colnames(taxTable_present())[3])
})
...
output$Sample_ContributionTer <- renderPlot({
  ...
  if (input$selectVariable_barplotTer != "unordered"){
    sampleCurveTer <-
      ggplot(data = sampleAnnot_orderedTer(), aes_string(x = "sampleName", y = input$selectVariable_barplotTer, group = 1)) +
      geom_point() +
      theme_bw() +
      geom_line() +
      theme(axis.text.x = element_text(angle = 90, hjust = 1))
    plot_grid(MEs_barplotTer, sampleCurveTer, labels = c("A", "B"), nrow = 2, align = "v")
  }else{
    MEs_barplotTer
  }
})
...
output$Relative_Abundance_ModuleTer <- renderPlot({
  ggplot(module_Relative_abundanceTer(), aes_string(x = "variable", y = "value", fill = input$selectTaxo2Ter)) +
    geom_bar(stat = "identity") +
...
output$Corr_External_TraitTer<- renderPlot({
  ggplot(data = selectedCondDFTer(), aes(x = rn, y = variable, fill = value)) +
    geom_tile(color = "white") +
    scale_fill_gradient2(low = "blue", high = "red", mid = "white",
                         midpoint = 0, limit = c(-1,1), space = "Lab",
                         name=paste("Module\n Conditions \nCorrelation"))+
...
output$Module_MembershipTer <- renderPlot({
  validate(
    need(length(unique(sampleAnnot_2()[, input$selectVariable1])) >1, "The external traits does not discriminate several conditions" )
  )
  ggplot(moduleMembershipTer(), aes(x = moduleMembership, y = TraitSignificance)) +
...
output$CVS_D3 <- renderText({
  print(cvs.object_D3())
})

output$ncomp_D3 <- renderPlot({
  plot(RMSEP(fit.object.ncomp_D3(), legendpos = "topright"))
})

output$summaryFit_D3 <- renderText({
...
output$edge_node_D3 <- renderPlot({
  ...
  if (length(unique(df_hive$annotation)) > 20){
    ggplot() +
      geom_point(data = df.hive.plot_D3(), aes(x1, y1, size = 3, color = "grey")) + # x1 is the VIP
      geom_text(data = df.hive.plot_D3(), vjust = 0.5, angle = 45, aes(x1, y1, label = annotation)) +
      geom_point(data = df.hive.plot_D3(), aes(x2, abs(y2), size = 3, color = "grey")) + # y2 is the correlation to the pH at time of filtering
      geom_text(data = df.hive.plot_D3(), hjust = 0.5, angle = 45, aes(x2, abs(y2), label = annotation)) + # y2 is the correlation to the pH at time of filtering
...
output$Download_Network_Exploration_dataset_3 <- downloadHandler(
  filename = function(){
    paste("NETWORK_EXPLORATION_dataset3.zip")
  },
  content = function (filename){
    fs <- c()
    tmpdir <- tempdir()
    setwd(tempdir())
    if (input$pdf_or_svg_p4_dataset3 == "pdf"){
      pdf("Contribution_MEs.pdf", width = input$widthPDF_D3, height = input$heightPDF_D3)
      for (i in 1:ncol(selectedMEs3())){
        print(barplot(selectedMEs3()[,i], col= substr(colnames(selectedMEs3())[i], 3, 40), main="", cex.main=2,
                      ylab="eigengene expression", cex.names = 0.7, names.arg = rownames(sampleAnnot_ter()), las = 2))
      }
      dev.off()
      fs <- c(fs, "Contribution_MEs.pdf")
      ...
      for (i in 1:ncol(selectedMEs3())){
        svg(paste("Contribution_MEs_", substr(colnames(selectedMEs3())[i], 3, 40), ".svg", sep = ""), width = input$widthPDF_D3, height = input$heightPDF_D3)
        print(barplot(selectedMEs3()[,i], col= substr(colnames(selectedMEs3())[i], 3, 40), main="", cex.main=2,
                      ylab="eigengene expression", cex.names = 0.7, names.arg = rownames(sampleAnnot_ter()), las = 2))
        fs <- c(fs, paste("Contribution_MEs_", substr(colnames(selectedMEs3())[i], 3, 40), ".svg", sep = ""))
      }
      svg(paste("Module_", input$selectModuleTer ,"_Corr_Vs_Membership.svg", sep=""), width = input$widthPDF_D3, height = input$heightPDF_D3)
      print(ggplot(moduleMembershipSec(), aes(x = moduleMembership, y = TraitSignificance)) +
              geom_point(color = input$selectModuleTer) +
              geom_smooth(method=lm, color = "white") +
              labs(x = paste("Module Membership in", input$selectModuleTer, "module"), y = paste("OTU significance for", input$selectVariable1Ter),
