...
exprDatTer <- reactive({
  ...
  if (input$Filtration2 == "Yes"){
    if (input$TypeFiltration2 == "prevalence"){
      if(input$prevalence != 0){
        Prevalence_threshold = round((input$prevalence2/100) * nrow(exprDat))
        names_OTU <- c()
        for (col in 1:ncol(exprDat)){
          nb_zero = 0
          for (row in 1:nrow(exprDat)){
  ...
  if(input$Normalisation2 == "Yes"){
    if (input$TypeNormalisation2 == 'CSS'){
      ...
      if (input$TypeNormalisation2 == 'TSS'){
        #data.TSS = t(apply(data.filter, 1, TSS.divide))
        exprDat <- apply(exprDat, 2, TSS.divide)
      }
    }
  ...
  if (input$Transformation2 == "Yes"){
    if (input$TypeTransformation2 == "Log10"){
      exprDat <- log((1 + exprDat), base = 10)
    }else{
      if(input$TypeTransformation2 == "Log2"){
        exprDat <- log((1 + exprDat), base = 2)
      }else{
        if(input$TypeTransformation2 == "Square"){
          exprDat <- exprDat^2
        }else{
          if(input$TypeTransformation2 == "sqrt"){
            exprDat <- sqrt(exprDat)
          }else{
            if(input$TypeTransformation2 == "Hellinger"){
              exprDat <- decostand(exprDat, method = "hellinger")
            }else{
              if (input$TypeTransformation2 == "CLR"){
                exprDat = clr(exprDat)
              }else{
                if (input$TypeTransformation2 == "ILR"){
                  exprDat.ilr = ilr(exprDat)
                  B <- exp(ilrBase(D=ncol(exprDat)))
                  exprDat = t(apply(exprDat.ilr, 1, function(x){
                    B[,1]^x[1] * B[,2]^x[2]
...
sampleAnnot_ter <- reactive({
  sampleAnnot <- sampleAnnot_sec()[which(rownames(sampleAnnot_sec()) %in% rownames(exprDatTer())),]
  sampleAnnot
})

sampleAnnot_ter_WGCNA <- reactive({
  sampleAnnot <- sampleAnnot()[which(rownames(sampleAnnot()) %in% rownames(exprDatTer_WGCNA())),]
  sampleAnnot <- sampleAnnot[match(rownames(sampleAnnot), rownames(exprDatTer_WGCNA())),]
  sampleAnnot
})


exprDatTer_2 <- reactive({
  expr <- exprDatTer()
  expr <- expr[which(rownames(expr) %in% rownames(sampleAnnot_ter())),]
  expr <- expr[match(rownames(expr), rownames(sampleAnnot_ter())),]
  expr
})
...
taxTable_Ter <- reactive({
  taxTable <- taxTable()
  taxTable <- taxTable[which(rownames(taxTable) %in% colnames(exprDatTer_2())),]
  taxTable <- taxTable[match(rownames(taxTable), colnames(exprDatTer_2())),]
  taxTable$rn <- colnames(exprDatTer_2())
  taxTable
})
...
exprDat_3 <- reactive({
  ...
  if (input$Omic3){
    ...
    expr3 <- expr3[which(rownames(expr3) %in% rownames(expr2)),]
    expr <- expr[which(rownames(expr) %in% rownames(expr3)),]
...
exprDat_41 <- reactive({
  if (input$selectModule12_p5 == "Individual_Variables"){
    ...
    exprDat_3()[,c(input$selectVariablesExprDat)]
  }else{
    modGenes = (selectedDynamicColor()== input$selectModule12_p5)
    exprDat_3()[,modGenes]
  }
})

exprDat_43 <- reactive({
  if (input$selectModule13_p5 == "Individual_Variables"){
    validate(
      need(input$selectVariablesExprDat != "", "Select variables to plot in the heatmap")
    )
    exprDat_3()[,c(input$selectVariablesExprDat)]
  }else{
    modGenes = (selectedDynamicColor()== input$selectModule13_p5)
    exprDat_3()[,modGenes]
  }
})

exprDatSec_51 <- reactive({
  if (input$selectModule21_p5 == "Individual_Variables"){
    validate(
...
exprDatSec_52 <- reactive({
  if (input$selectModule23_p5 == "Individual_Variables"){
    validate(
      need(input$selectVariables != "", "Select variables to plot in the heatmap")
    )
...
exprDatTer_52 <- reactive({
  if (input$selectModule32_p5 == "Individual_Variables"){
    validate(
      need(input$selectVariables != "", "Select variables to plot in the heatmap")
    )
...
exprDatTer_53 <- reactive({
  if (input$selectModule31_p5 == "Individual_Variables"){
    validate(
      need(input$selectVariables != "", "Select variables to plot in the heatmap")
    )
    exprDatTer_3()[,c(input$selectVariablesExprDat)]
...
output$ViewTable <- renderDataTable({
  ...
  if(is.null(input$View1)&is.null(input$View2)&is.null(input$View3)&is.null(input$View4)&is.null(input$View5)&is.null(input$View6)){
    ...
    if(input$TypeAnalysis == 'multivariate'){
      ...
      if (input$Omic3){
        validate(
          need(input$file5$datapath != "", "Please upload a Third omic table file")
        )
        validate(
          ...
          if (input$View6 == "counting"){
            ...
            if (input$View6 == "annot"){
              ...
              if (input$View6 == "taxa"){
                ...
                if (input$View6 == "ter"){
                  dimension <- as.data.frame(dim(exprDatTer_present()))
                  colnames(dimension) <- c("dimension")
                  rownames(dimension) <- c("row", "col")
                  datatable(dimension,
                            options = list(lengthMenu = FALSE, dom = 'tip'),
